...
class InventoryItemControllerTest {
    ...
    // ========================================
    // CHECK NAME AVAILABILITY TESTS
    // ========================================

    @Nested
    @DisplayName("Check Name Availability Tests - GET /item/check-name")
    class CheckNameAvailabilityTests {

        @Test
        @DisplayName("checkName_withTakenName_returnsOk200True")
        void checkName_withTakenName_returnsOk200True() throws Exception {
            // Given
            String name = "Steel Bolt M10";

            when(service.isNameTaken(name)).thenReturn(true);

            // When & Then
            mockMvc.perform(get("/item/check-name")
                    .param("name", name))
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(content().string("true"));

            verify(service).isNameTaken(name);
        }

        @Test
        @DisplayName("checkName_withAvailableName_returnsOk200False")
        void checkName_withAvailableName_returnsOk200False() throws Exception {
            // Given
            String name = "New Unique Item";
        ...
        @Test
        @DisplayName("checkName_withBlankName_returnsBadRequest400")
        void checkName_withBlankName_returnsBadRequest400() throws Exception {
            // When & Then
            mockMvc.perform(get("/item/check-name")
                    .param("name", "   "))  // Blank name
                .andExpect(status().isBadRequest());

            verify(service, never()).isNameTaken(anyString());
        }

        @Test
        @DisplayName("checkName_withMissingParameter_returnsBadRequest400")
        void checkName_withMissingParameter_returnsBadRequest400() throws Exception {
            // When & Then
            mockMvc.perform(get("/item/check-name"))  // Missing 'name' parameter
                .andExpect(status().isBadRequest());
